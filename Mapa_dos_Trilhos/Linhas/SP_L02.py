...
ARQUITETURA:
    Mapa_dos_Trilhos/Linhas/SP_L02.py
"""
import os
import json
import sys
...
def line2():
    try:
        ...
        print(f"Executando linha 2: {sys.executable} {script_path}")
        ...
        print("Linha 2 executada com sucesso!")
        return True
        
    except subprocess.CalledProcessError as e:
        error_msg = f"Erro ao executar linha 2:\n{e.stderr}"
        print(error_msg)
        logging.error(error_msg)
        return False
    except Exception as e:
        error_msg = f"Erro inesperado em line2: {str(e)}"
        print(error_msg)
        logging.error(error_msg)
        return False
...
class MapaLinhaWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Linha 2 - Verde")
        self.setWindowIcon(QIcon('Mapa_dos_Trilhos/Favicon/2_verde.ico'))
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape:
            with open('Mapa_dos_Trilhos/log.log', 'a', encoding='utf-8') as f:
                f.write(f"{datetime.now()} - Fechando Linha 2 - Verde\n")
            self.close()
