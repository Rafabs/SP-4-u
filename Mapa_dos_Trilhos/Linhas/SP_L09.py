...
ARQUITETURA:
    Mapa_dos_Trilhos/Linhas/SP_L09.py
"""
import os
import json
import sys
...
def line9():
    try:
        ...
        print(f"Executando linha 9: {sys.executable} {script_path}")
        ...
        print("Linha 9 executada com sucesso!")
        return True
        
    except subprocess.CalledProcessError as e:
        error_msg = f"Erro ao executar linha 9:\n{e.stderr}"
        print(error_msg)
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape:
            with open('Mapa_dos_Trilhos/log.log', 'a', encoding='utf-8') as f:
                f.write(f"{datetime.now()} - Fechando Linha 9 - Esmeralda\n")
            self.close()
