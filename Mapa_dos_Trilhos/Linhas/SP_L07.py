...
ARQUITETURA:
    Mapa_dos_Trilhos/Linhas/SP_L07.py
"""
import os
import json
import sys
...
def line7():
    try:
        ...
        print(f"Executando linha 7: {sys.executable} {script_path}")
        ...
        print("Linha 7 executada com sucesso!")
        return True
        
    except subprocess.CalledProcessError as e:
        error_msg = f"Erro ao executar linha 7:\n{e.stderr}"
        print(error_msg)
...
class MapaLinhaWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.status_linha = self.status_manager.get_line_status("Linha 7 - Rubi")
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape:
            with open('Mapa_dos_Trilhos/log.log', 'a', encoding='utf-8') as f:
                f.write(f"{datetime.now()} - Fechando Linha 7 - Rubi\n")
            self.close()
