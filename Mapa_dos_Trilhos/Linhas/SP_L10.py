...
__modified__    = "28/06/2025 16:55"

DESCRITIVO:
MÃ³dulo de funcionalidades especÃ­ficas
ARQUITETURA:
    Mapa_dos_Trilhos/Linhas/SP_L10.py
"""
import os
import json
import sys
...
def line10():
    try:
        ...
        print(f"Executando linha 10: {sys.executable} {script_path}")
        ...
        print("Linha 10 executada com sucesso!")
        return True
        
    except subprocess.CalledProcessError as e:
        error_msg = f"Erro ao executar linha 10:\n{e.stderr}"
        print(error_msg)
        logging.error(error_msg)
        return False
    except Exception as e:
        error_msg = f"Erro inesperado em line10: {str(e)}"
        print(error_msg)
        logging.error(error_msg)
        return False
...
class MapaLinhaWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Linha 10 - Turquesa")
        self.setWindowIcon(QIcon('Mapa_dos_Trilhos/Favicon/10_turquesa.ico'))
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape:
            logging.info(f"Fechando Linha 10 - Turquesa")
            self.close()
