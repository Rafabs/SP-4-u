...
__modified__    = "08/07/2025 01:34"

DESCRITIVO:
MÃ³dulo de funcionalidades especÃ­ficas
ARQUITETURA:
    Mapa_dos_Trilhos/Linhas/SP_L03.py
"""
import os
import json
import sys
...
def line3():
    try:
        ...
        print(f"Executando linha 3: {sys.executable} {script_path}")
        ...
        print("Linha 3 executada com sucesso!")
        return True
        
    except subprocess.CalledProcessError as e:
        error_msg = f"Erro ao executar linha 3:\n{e.stderr}"
        print(error_msg)
        logging.error(error_msg)
        return False
    except Exception as e:
        error_msg = f"Erro inesperado em line3: {str(e)}"
        print(error_msg)
        logging.error(error_msg)
        return False
...
class MapaLinhaWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Linha 3 - Vermelha")
        self.setWindowIcon(QIcon('Mapa_dos_Trilhos/Favicon/3_vermelha.ico'))
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape:
            with open('Mapa_dos_Trilhos/log.log', 'a', encoding='utf-8') as f:
                f.write(f"{datetime.now()} - Fechando Linha 3 - Vermelha\n")
            self.close()
