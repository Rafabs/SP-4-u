...
__modified__    = "28/06/2025 16:08"

DESCRITIVO:
    MÃ³dulo de funcionalidades especÃ­ficas

ARQUITETURA:
    Mapa_dos_Trilhos/Linhas/SP_L12.py
"""
import os
import json
import sys
...
def line12():
    try:
        ...
        print(f"Executando linha 12: {sys.executable} {script_path}")
        ...
        print("Linha 12 executada com sucesso!")
        return True
        
    except subprocess.CalledProcessError as e:
        error_msg = f"Erro ao executar linha 12:\n{e.stderr}"
        print(error_msg)
        logging.error(error_msg)
        return False
    except Exception as e:
        error_msg = f"Erro inesperado em line12: {str(e)}"
        print(error_msg)
        logging.error(error_msg)
        return False
...
class MapaLinhaWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Linha 12 - Safira")
        self.setWindowIcon(QIcon('Mapa_dos_Trilhos/Favicon/12_safira.ico'))
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape:
            logging.info(f"Fechando Linha 12 - Safira")
            self.close()
